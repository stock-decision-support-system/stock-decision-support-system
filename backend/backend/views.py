...
def register(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            return Response(
                {"status": "error", "message": "Unable to register user."},
                status=status.HTTP_400_BAD_REQUEST,
            )
    else:
        return Response(
...
def password_reset_request(request):
    if request.method == "POST":
        ...
        try:
            ...
            return Response(
                {"status": "error", "message": "No account with that email."},
                status=status.HTTP_400_BAD_REQUEST,
            )
    return Response(
        {"status": "error", "message": "Invalid request method."},
...
def accounting_list_for_user(request):
    if request.method == "GET":
        ...
        accountings = Accounting.objects.filter(
            createdId=create_id, available=True
        ...
        if request.method == "PUT":
            ...
            request.method == "DELETE" and request.user.username == accounting.createdId
        ):
            try:
                # Use .update() for QuerySets
                updated = Accounting.objects.filter(accountingId=pk).update(
                    available=False
                )
                if updated:
                    return Response(
                        {"message": "Accounting record has been soft-deleted."},
...
def consume_type_operations(request, pk=None):
    ...
    #             )
    #             return Response(
    #                 {'status': 'success', 'message': 'Password reset email has been sent.', 'token': token, 'uid': uid},
    #                 status=status.HTTP_200_OK)
    #         except CustomUser.DoesNotExist:
    #             return Response({'status': 'error', 'message': 'No account with that email.'},
    #                             status=status.HTTP_400_BAD_REQUEST)
    ...
    #
    #     return Response(
    #         {'status': 'success', 'email': user.email, 'first_name': user.first_name, 'last_name': user.last_name,
    #          'username': username})
    #
    # # 修改個人帳戶資訊
    # @api_view(['POST'])
...
def get_bank_profile_list(request):
    if request.method == "GET":
        ...
        return Response(
            {"status": "success", "users": bank_data}
        )
